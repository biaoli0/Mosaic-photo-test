...
The SvelteKit **frontend** for the mosaic photo generator. The image processing itself runs in a separate Node service — see [`../mosaic-backend`](../mosaic-backend). This package is now UI-only: it decodes the file, splits it into chunks, calls the backend over HTTP for each chunk, and paints the responses progressively onto a canvas.

## Prerequisites

- Node.js 18+
- The `mosaic-backend` service running (default `http://localhost:3001`).
...
| -------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `VITE_MOSAIC_BACKEND_URL`  | `http://localhost:3001` | Resolved at build time. Must be set in the build environment for production. Vite-exposed env vars must be prefixed `VITE_`. |
...
- **Chunked rendering.** A bitmap is decoded once on the main thread, then `chunkPlanner` slices it into tile-aligned horizontal strips and a small worker pool (default concurrency 4) sends each strip to `POST {VITE_MOSAIC_BACKEND_URL}/mosaic?tileSize=<n>`. As each strip comes back it's drawn into the visible canvas, giving a top-to-bottom progressive reveal.
