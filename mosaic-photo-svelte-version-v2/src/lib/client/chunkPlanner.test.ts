...
describe('planChunks', () => {
	...
	it('makes the trailing chunk shorter when height is not a multiple of chunkBaseHeight', () => {
		...
		expect(plans.map((p) => p.y)).toEqual([0, 100, 200]);
		expect(plans.map((p) => p.height)).toEqual([100, 100, 50]);
	});
	...
	it('rounds chunk height down to a multiple of tileSize to avoid seams', () => {
		...
		expect(plans.map((p) => p.y)).toEqual([0, 96]);
		expect(plans.map((p) => p.height)).toEqual([96, 96]);
	});
	...
	it('falls back to tileSize when the target chunk height is smaller than the tile', () => {
		...
		expect(plans.map((p) => p.y)).toEqual([0, 200]);
		expect(plans.map((p) => p.height)).toEqual([200, 200]);
	});
